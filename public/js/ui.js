...
export default class UIManager {
    ...
    async init() {
        ...
        try {
            ...
            try {
                ...
                console.warn(" Continuando sin perfiles (solo detectará 'Desconocido')");
                this._updateStatus(' Sin perfiles - solo detección básica');
            ...
            if (!this.backgroundMode) {
                ...
                console.log("Paso 6/7: Saltando cámaras locales (background mode)");
            }
            ...
            if (useWS) {
                ...
                console.log(" WebRTC deshabilitado (modo local)");
            }
            ...
            if (this.videos.length > 0 || this.backgroundMode) {
                ...
                console.log(" No hay videos, detección no iniciada");
            }
            ...
            if (this.thresholdInput) {
                this.thresholdInput.addEventListener('input', () => {
                    ...
                    console.log(" Umbral ajustado a:", v);
                });
            }
    ...
    async _loadProfileThumbs() {
        try {
            ...
            if (!res.ok) {
                console.warn(" No se pudieron cargar thumbnails");
                return;
            }

            const profiles = await res.json();
            console.log(` Procesando ${profiles.length} thumbnails`);
            ...
            console.warn(' Error cargando miniaturas:', e.message);
        }
    }
    ...
    async _createLocalCameras() {
        ...
        if (!this.container) {
            console.warn("Container no encontrado, saltando creación de feeds locales");
            return;
        }
    ...
    async _syncCameraWithBackend(deviceId, name) {
        try {
            ...
            console.log(`Cámara sincronizada: ${deviceId}`);

        } catch (err) {
            console.warn("Error sincronizando cámara:", err.message);
        }
    }
    ...
    try {
        ...
        console.log(`${cameras.length} cámaras cargadas desde backend`);

    } catch (err) {
        console.warn("Error cargando cámaras:", err.message);
    }
}
    ...
    _createVideoCanvasPair(id, stream, opts = {}) {
        // VALIDACIÓN: Solo crear si hay container
        if (!this.container) {
            console.warn(`Container no encontrado, no se puede crear feed ${id}`);
            return null;
        }

        if (document.getElementById(id)) {
            console.warn(`Feed ${id} ya existe`);
            return null;
        }
    ...
    async _startAutoDetection() {
        ...
        if (this.backgroundMode && this.videos.length === 0) {
            ...
            setTimeout(() => {
                clearInterval(checkInterval);
                if (this.videos.length === 0) {
                    console.warn("No se detectaron feeds remotos después de 30s");
                }
            }, 30000);
